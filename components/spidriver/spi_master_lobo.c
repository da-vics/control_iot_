...
static esp_err_t spi_lobo_bus_initialize(spi_lobo_host_device_t host, spi_lobo_bus_config_t *bus_config, int init)
{
    ...
    if (init > 0)
    {
        ...
        if (!spihost[host]->spi_lobo_bus_mutex)
            return ESP_ERR_NO_MEM;
    }
    ...
    if (bus_config->mosi_io_num >= 0 && bus_config->mosi_io_num != io_signal[host].spid_native)
        native = false;
    if (bus_config->miso_io_num >= 0 && bus_config->miso_io_num != io_signal[host].spiq_native)
        native = false;
    if (bus_config->sclk_io_num >= 0 && bus_config->sclk_io_num != io_signal[host].spiclk_native)
        native = false;
    if (bus_config->quadwp_io_num >= 0 && bus_config->quadwp_io_num != io_signal[host].spiwp_native)
        native = false;
    if (bus_config->quadhd_io_num >= 0 && bus_config->quadhd_io_num != io_signal[host].spihd_native)
        native = false;
...
{
    if ((host == TFT_SPI_HOST) || (host > TFT_VSPI_HOST))
        return ESP_ERR_NOT_SUPPORTED; // invalid host

    if (spihost[host] == NULL)
        return ESP_ERR_INVALID_STATE; // host not in use

    if (dofree)
    {
        for (int x = 0; x < NO_DEV; x++)
        {
            if (spihost[host]->device[x] != NULL)
                return ESP_ERR_INVALID_STATE; // not all devices freed
        }
    }
...
esp_err_t spi_lobo_bus_add_device(spi_lobo_host_device_t host, spi_lobo_bus_config_t *bus_config, spi_lobo_device_interface_config_t *dev_config, spi_lobo_device_handle_t *handle)
{
    ...
    {
        if (!GPIO_IS_VALID_OUTPUT_GPIO(dev_config->spics_io_num))
    ...
    for (freecs = 0; freecs < maxdev; freecs++)
    {
        ...
        if (__sync_bool_compare_and_swap(&spihost[host]->device[freecs], NULL, (spi_lobo_device_t *)1))
            break;
    }
    if (freecs == maxdev)
    ...
    if ((dev_config->cs_ena_pretrans != 0) && (dev_config->flags & LB_SPI_DEVICE_HALFDUPLEX))
        return ESP_ERR_INVALID_ARG;

    // Speeds >=40MHz over GPIO matrix needs a dummy cycle, but these don't work for full-duplex connections.
    if (((dev_config->flags & LB_SPI_DEVICE_HALFDUPLEX) == 0) && (dev_config->clock_speed_hz > ((apbclk * 2) / 5)) && (!spihost[host]->no_gpio_matrix))
        return ESP_ERR_INVALID_ARG;
...
esp_err_t IRAM_ATTR spi_lobo_device_select(spi_lobo_device_handle_t handle, int force)
{
    ...
    if (!(xSemaphoreTake(host->spi_lobo_bus_mutex, SPI_SEMAPHORE_WAIT)))
        return ESP_ERR_INVALID_STATE;
...
esp_err_t IRAM_ATTR spi_lobo_device_deselect(spi_lobo_device_handle_t handle)
{
    ...
    if (handle->cfg.selected == 0)
        return ESP_OK; // already deselected
...
{
    if (!(xSemaphoreTake(handle->host->spi_lobo_bus_mutex, SPI_SEMAPHORE_WAIT)))
        return ESP_ERR_INVALID_STATE;
    else
        return ESP_OK;
}
...
uint32_t spi_lobo_get_speed(spi_lobo_device_handle_t handle)
{
    ...
    if (spi_lobo_device_select(handle, 0) == ESP_OK)
    {
        if (host->hw->clock.clk_equ_sysclk == 1)
            ...
            speed = 80000000 / (host->hw->clock.clkdiv_pre + 1) / (host->hw->clock.clkcnt_n + 1);
    }
    ...
    if (spi_lobo_device_select(handle, 0) == ESP_OK)
    {
        ...
        if (spi_lobo_device_select(handle, 1) == ESP_OK)
        {
            if (host->hw->clock.clk_equ_sysclk == 1)
                ...
                newspeed = 80000000 / (host->hw->clock.clkdiv_pre + 1) / (host->hw->clock.clkcnt_n + 1);
        }
    }
    spi_lobo_device_deselect(handle);
    ...
    if (txlen > 0)
    {
        ...
        while (count < txlen)
        {
            ...
            for (bc = 0; bc < 32; bc += 8)
            {
                ...
                if (count == txlen)
                    break; // If all transmit data pushed to hw spi buffer break from the loop
            }
